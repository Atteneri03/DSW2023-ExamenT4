<?php include "top.php"; ?>
    ...
    <section id="films">
        ...
        if(isset($_GET["category"]) && isset($_GET["search"])){
            $id = $_GET["category"];
            $sql = "SELECT film.film_id,film.title, film.description, film.release_year, film.length FROM film, film_category WHERE film.film_id = film_category.film_id AND film_category.category_id = $id;";
            $result = $link->query($sql);
            $film = $result->fetch_assoc();
            if ($film == null) {
                echo '<h3>No hay peliculas</h3>';
            } else {
